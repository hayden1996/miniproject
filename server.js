		...
		db.collection("user").find(whereStr).toArray(function (err, result) {
			...
			if (result.length > 0) {
				if (result[0].password == userpwd) {
					...
					// var result=`<script>alert('login successful');location.href="home"; </script>`;
					// 	 res.send(result);
res.render('redirect.ejs', { link: "home", text :'login successful'})
...
app.post('/process_register', urlencodedParser, function (req, res) {
	...
	client.connect((err) => {
		...
		insertUser(db, new_user, (result) => {
			...
			res.render('message.ejs', {
				message: "User was inserted into MongoDB!",
				BackLink: "home",
				BackText: "go back",
			})
		})
			...
			db.collection("test").find(whereStr).toArray(function (err, result) {
				...
				if (result.length > 0) {
					...
					if (result[0].owner == req.session.username) {
						...
						res.render('message.ejs', {
							message: "you not onwer cannot edit",
							BackLink: "home",
							BackText: "go back",
						})
